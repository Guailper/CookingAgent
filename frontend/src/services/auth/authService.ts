...
function toAuthServiceError(payload: ApiErrorResponse | null, status: number): AuthServiceError {
  ...
  switch (code) {
    ...
    case "EMAIL_CODE_TOO_FREQUENT":
      return new AuthServiceError(code, "验证码发送过于频繁", backendMessage);
    case "EMAIL_CODE_INVALID_OR_EXPIRED":
    case "EMAIL_CODE_LOCKED":
      return new AuthServiceError(code, "验证码不可用", backendMessage);
    case "EMAIL_SERVICE_NOT_CONFIGURED":
    case "EMAIL_SEND_FAILED":
    case "EMAIL_SERVER_UNREACHABLE":
      return new AuthServiceError(code, "验证码邮件发送失败", backendMessage);
    case "AUTH_REQUIRED":
...
export async function loginWithEmailCode(input: EmailCodeLoginInput): Promise<AuthSession> {
  const email = normalizeEmail(input.email);
  const emailCode = input.emailCode.trim();

  if (!email || !emailCode) {
    throw new AuthServiceError(
      "missing_email_code_login_fields",
      "缺少登录信息",
      "请输入邮箱和验证码后再登录。",
    );
  }

  const response = await requestJson<ApiAuthResponse>("/auth/email-code/login", {
    method: "POST",
    headers: getAuthHeaders(),
...
export async function registerAccount(input: RegisterInput): Promise<AuthSession> {
  ...
  if (!fullName || !email || !password || !confirmPassword || !emailCode) {
    throw new AuthServiceError(
      "missing_fields",
      "请完善表单",
      "创建账号前需要填写所有字段，并完成邮箱验证码校验。",
    );
  }
...
export async function sendEmailCode(email: string, purpose: EmailCodePurpose) {
  ...
  await requestJson<{ message: string }>("/auth/email-code/send", {
    method: "POST",
    headers: getAuthHeaders(),
    body: JSON.stringify({
